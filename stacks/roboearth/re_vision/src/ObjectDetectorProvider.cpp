...
boost::shared_ptr<const sensor_msgs::Image> ros_img_ptr
  ...
  //img_bridge.image = new sensor_msgs::Image(req.Image);
  ...
  if(m_camera.usesBayer())
  {
    try {
       ...
       //sensor_msgs::ImagePtr ros_img = img_bridge.toImageMsg();
       //im = sensor_msgs::Image(ros_img);
    }
    catch (cv_bridge::Exception& e) {
       ROS_ERROR ("Could not convert from '%s' to 'bgr8'.", ros_img_ptr->encoding.c_str ());
    }
  }
  else
  {
    try {
       ...
       //sensor_msgs::ImagePtr ros_img_ptr = img_bridge.toImageMsg();
       //im = sensor_msgs::Image(ros_img);
    }
    catch (cv_bridge::Exception& e) {
       ROS_ERROR ("Could not convert from '%s' to 'bgr8'.", ros_img_ptr->encoding.c_str ());
