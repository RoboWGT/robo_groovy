  ...
  for(int i=0; i<6; i++) {
	...
	*(transformations[i][j][k]) = CoordinateFrame(&p1, &p2, &p3);
    ...
    void ARPublisher::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
    {
        ...
        try {
           capture_ = cv_bridge::toCvCopy (image_, sensor_msgs::image_encodings::BGR8);
        }
...
//        cvShowImage("win",capture_);
        ...
        dataPtr = (ARUint8 *) ((IplImage) capture_->image).imageData;//= (ARUint8 *) capture_->imageData;
        ...
        int downsize = ((IplImage) capture_->image).width/cloud_width_;//capture_->width/cloud_width_;
